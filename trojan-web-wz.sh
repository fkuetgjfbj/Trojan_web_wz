...
if test -s /etc/nginx/nginx.conf; then
  ...
  wget -P /etc/nginx https://raw.githubusercontent.com/siropboy/Trojan_web_wz/master/nginx.conf
	green "================================="
	blue "     请输入Trojan绑定的域名"
	green "================================="
	read ni_domain
  sed -i "s/localhost/$ni_domain/;" /etc/nginx/nginx.conf
	green " "
	green "================================="
	 blue "    开始下载伪装站点源码并部署"
	green "================================="
	...
	wget https://github.com/siropboy/Trojan_web_wz/raw/master/web/web.zip
	unzip web.zip
	green " "
	green "================================="
	blue "       开始配置trojan-web-wz"
	green "================================="
	sleep 2s
  ...
  green " "
	...
	green " " 
	green " "
	 blue "  更改 Trojan 面板端口并设置伪装站点"
	green " "
	 blue "  伪装站点目录 /usr/share/nginx/html "
	 blue "  面板管理地址 http://$your_domain:88 "
	green "=================================================================="
else
	green "==============================="
	  red "     Nginx未正确安装 请重试"
